...
**The floor you can't see from files.** The scan sums file estimates, but the heaviest always-loaded categories often aren't files at all: MCP tool schemas (verbose JSON Schema, pulled from the *running* server — `.mcp.json` names the server but holds none of the schema), the base system prompt, and built-in tool definitions. A file scan is structurally blind to these, and MCP in particular is frequently the single largest block. So treat the scan total as a floor, not the whole. Have the user run `/context` (or the host's equivalent) and reconcile against it: the gap between `/context`'s always-loaded number and your scan total is mostly MCP + system+tools. That residual is exactly the number the user will compare you against — and it's usually where the real bloat lives, not in the markdown you can read. One caveat on the numbers themselves: the script's estimates are `÷4` (chars/token) and will drift from `/context`'s real tokenizer. That's fine for *ranking* files, but never present a `÷4` total as if it reconciles with `/context` — cite `/context` for headline always-loaded figures.
